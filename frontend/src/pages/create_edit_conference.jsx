...
export default function CreateEditConference() {
  ...
  const { role } = useAuth();
  const dashboardPath = role === "admin" ? "/admin-dashboard" : "/organiser-dashboard";
  ...
  const submit = async (event) => {
    ...
    if (!form.code.trim() || !form.name.trim() || !form.description.trim() || !form.category.trim() || !form.start_date || !form.end_date || !form.submission_deadline || !form.city.trim() || !form.country.trim()) {
      setError("Please complete all required conference details.");
      setSaving(false);
      return;
    }
    if (start < today) {
      setError("Conference date must be today or in the future.");
      setSaving(false);
      return;
    }
    if (end < start) {
    ...
    if (deadline > start) {
      setError("Submission deadline must be on or before the conference start date.");
      setSaving(false);
      return;
    }
    ...
    <div className="min-h-screen bg-[#f7f9fc] text-[#0d1b3d] dark:bg-[#07132f] dark:text-white">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-[#07132f]/95 text-white shadow-[0_8px_30px_rgba(7,19,47,.12)] backdrop-blur-xl">
        <div className="mx-auto flex min-h-[76px] w-[min(1400px,calc(100%-32px))] items-center justify-between gap-4">
          <button onClick={() => navigate(dashboardPath)} className="inline-flex items-center gap-2 text-[12px] font-bold text-white/75 transition hover:text-white" type="button"><ArrowLeft size={16} /> Conference workspace</button>
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(dashboardPath)} type="button" className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-white/15 px-3 text-[11px] font-bold text-white/80 transition hover:bg-white/10 hover:text-white"><Edit3 size={14} /> <span className="hidden sm:inline">Back to workspace</span><span className="sm:hidden">Edit</span></button>
