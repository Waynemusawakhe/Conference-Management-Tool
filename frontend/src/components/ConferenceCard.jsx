import { CalendarDays, MapPin, ArrowUpRight, Clock, FileUp } from "lucide-react";
    ...
    <article className={`overflow-hidden rounded-2xl border border-[#e5e8ef] bg-white shadow-[0_12px_32px_rgba(15,28,65,.06)] transition hover:-translate-y-1.5 hover:shadow-[0_22px_45px_rgba(15,28,65,.10)] ${isList ? "md:flex" : ""}`}>
      ...
      <div className={`p-5 ${isList ? "md:flex-1" : ""}`}>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-4">
          <button
            className="inline-flex items-center gap-1.5 border-0 bg-transparent p-0 text-[11px] font-extrabold text-[#5a4df1]"
            type="button"
          ...
          <button
            className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-br from-[#6655f6] to-[#7869ff] px-3 py-2 text-[11px] font-bold text-white shadow-[0_10px_26px_rgba(103,87,245,.26)] transition hover:-translate-y-0.5"
            type="button"
            onClick={() => navigate(`/conferences/${conference.id}/submit-proposal`)}
          >
            <FileUp size={14} /> Submit a Proposal
          </button>
        </div>
      </div>
    </article>
